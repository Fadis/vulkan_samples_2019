...
void main()  {
  ...
  vec3 V = ts * normalize(/*dynamic_uniforms*/dynamic_uniforms.eye_pos.xyz-pos);
  vec3 L = ts * normalize(/*dynamic_uniforms*/dynamic_uniforms.light_pos.xyz-pos);
  ...
  vec4 diffuse_color = texture( shadow/*base_color*/, gl_FragCoord.xy * vec2( 1.0/1920, 1.0/1080 )/*input_texcoord*/ ) * uniforms.base_color;
  ...
  vec3 linear = light( L, V, N, diffuse_color.rgb, roughness, metallicness, ambient, emissive, /*dynamic_uniforms*/dynamic_uniforms.light_energy );
